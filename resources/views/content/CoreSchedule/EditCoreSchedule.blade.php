...
<div class="card border border-dark">
    ...
    <form method="post" action="{{route('process-edit-schedule')}}" enctype="multipart/form-data">
        @csrf
        <div class="card-body">
            <div class="row form-group">
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <select class="selection-search-clear select-form" name="location_id" id="location_id">
                            <option value="{{$core_schedule->location_id}}">{{$CS->getLocationName($core_schedule->location_id)}}</option>
                            @foreach ($core_location as $location_id => $val)
                                <option value="{{$location_id}}">{{$val}}</option>
                            @endforeach
                            </select>
                            <input class="form-control input-bb" type="hidden" name="schedule_id" id="schedule_id" value="{{$core_schedule->schedule_id}}" autocomplete="off"/>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <select class="selection-search-clear select-form" name="schedule_day" id="schedule_day">
                            <option value="{{$core_schedule->schedule_day}}">{{$day[$core_schedule->schedule_day]}}</option>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input class="form-control input-bb" type="text" name="schedule_description" id="schedule_description" value="{{$core_schedule->schedule_description}}" autocomplete="off"/>
                    </div>
                </div>
            </div>
